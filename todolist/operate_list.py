...
def operate_list(server, info, args):
    if len(args) == 1: # !!td
        ...
        fun.save()
        if is_new:
            server.reply(info, f'§b[ToDoList]§a Added ToDo {args[2]}')
        else:
            server.reply(info, f'§b[ToDoList]§a Updated ToDo {args[2]}')
        ...
        if len(args) == 2: # !!td tag back to tag list
            ...
            if args[2] == "add" or args[2] == "a": # !!td tag add <name> <tag>
                if args[4] not in fun.list_dic.get(args[3])["tags"]:
                    ...
                    fun.save()
                    server.reply(info, f'§b[ToDoList]§a Tag {args[4]} has been added to ToDo {args[3]}')
                else:
                    server.reply(info, f'§b[ToDoList]§4 ToDo {args[3]} has tag {args[4]} already')
                ...
                if args[4] != "defult":
                    ...
                    if found_oldtag:
                        ...
                        fun.save()
                        server.reply(info, f'§b[ToDoList]§a Deleted tag {args[4]} for ToDo {args[3]}')
                    else:
                        server.reply(info, f'§b[ToDoList]§4 ToDo {args[3]} have no tag {args[4]}')
                else:
                    server.reply(info, f'§b[ToDoList]§4 Can\'t delete defult tag')
            else:
        ...
        if args[1] == "list" or args[1] == "l": # !!td list
            ...
            try:
                fun.read()
                server.say('§b[ToDoList]§a Reload success by player §r{}§a'.format(info.player))
            except Exception as e:
                server.say('§b[ToDoList]§4 Reload faild by player §r{}§a：{}'.format(info.player, e))
        ...
        if args[1] == "del" or args[1] == "d": # !!td del <name>
            ...
            if name:
                ...
                fun.save()
                server.reply(info, f'§b[ToDoList]§a Deleted {name}')
            else:
                server.reply(info, f"§b[ToDoList]§4 There\'s no §d{args[2]} §4 exists")
        else:
            server.reply(info, wrong_command)
    else:
        server.reply(info, wrong_command)
